...
async function createUploadToken(orderId, env) {
  ...
  const secret = env.ORDER_TOKEN_SECRET || env.STRIPE_WEBHOOK_SECRET || "development-only";
  const sig = await hmacHex(secret, `${orderId}.${exp}`);
...
async function verifyUploadToken(orderId, token, env) {
  ...
  const secret = env.ORDER_TOKEN_SECRET || env.STRIPE_WEBHOOK_SECRET || "development-only";
  const expected = await hmacHex(secret, `${orderId}.${exp}`);
...
async function uploadOrderAsset(request, env, url) {
  ...
  if (!orderId || !["outside", "inside"].includes(kind)) return json({ error: "Invalid asset request" }, 400);
...
async function stripeRequest(path, env, params) {
  if (!env.STRIPE_SECRET_KEY) throw new Error("Stripe is not connected");
...
async function createCheckout(request, env) {
  if (!env.DB) return json({ error: "Order database is not connected yet." }, 503);
  if (!env.STRIPE_SECRET_KEY) return json({ error: "Stripe is not connected yet. Finish the Stripe registration and add the live key." }, 503);
  const body = await parseJsonLimited(request);
  ...
  const totalName = order.art_mode === "photo" ? "Personalised AI Monster Card — Photo Edition" : "Personalised AI Monster Card";
  const session = await stripeRequest("/checkout/sessions", env, {
    ...
    cancel_url: `${base}/?checkout=cancelled#results`,
    client_reference_id: orderId,
    customer_creation: "always",
    "shipping_address_collection[allowed_countries][]": "GB",
    "line_items[0][price_data][currency]": "gbp",
    "line_items[0][price_data][unit_amount]": order.price_pence,
    "line_items[0][price_data][product_data][name]": totalName,
    "line_items[0][quantity]": 1,
    "line_items[1][price_data][currency]": "gbp",
    "line_items[1][price_data][unit_amount]": order.shipping_pence,
    "line_items[1][price_data][product_data][name]": "UK delivery",
    "line_items[1][quantity]": 1,
    "metadata[order_id]": orderId,
    "payment_intent_data[metadata][order_id]": orderId,
    "custom_text[submit][message]": "Your card is personalised and made to order. Please check the wording and delivery address before paying.",
  });

  await env.DB.prepare("UPDATE orders SET status='checkout', stripe_session_id=?, updated_at=? WHERE id=?")
...
async function makePrintAssetUrl(orderId, kind, env) {
  const exp = Math.floor(Date.now() / 1000) + 60 * 60 * 24;
  const secret = env.PRINT_ASSET_SECRET || env.ORDER_TOKEN_SECRET || env.STRIPE_WEBHOOK_SECRET;
  if (!secret) throw new Error("PRINT_ASSET_SECRET is missing");
  const sig = await hmacHex(secret, `${orderId}.${kind}.${exp}`);
  const base = env.PUBLIC_BASE_URL || "https://builttooffend.com";
  return `${base}/api/print-asset/${encodeURIComponent(orderId)}/${encodeURIComponent(kind)}?exp=${exp}&sig=${sig}`;
}
...
async function servePrintAsset(env, orderId, kind, url) {
  ...
  if (!["outside", "inside"].includes(kind)) return new Response("Not found", { status: 404 });
  const exp = Number(url.searchParams.get("exp"));
  const sig = url.searchParams.get("sig") || "";
  if (!Number.isFinite(exp) || exp < Math.floor(Date.now() / 1000)) return new Response("Expired", { status: 403 });
  const secret = env.PRINT_ASSET_SECRET || env.ORDER_TOKEN_SECRET || env.STRIPE_WEBHOOK_SECRET;
  if (!secret) return new Response("Unavailable", { status: 503 });
  const expected = await hmacHex(secret, `${orderId}.${kind}.${exp}`);
  if (!timingSafeStringEqual(sig, expected)) return new Response("Forbidden", { status: 403 });
  const order = await env.DB.prepare("SELECT outside_asset_key,inside_asset_key,status FROM orders WHERE id=?").bind(orderId).first();
  if (!order || !["paid", "submitted", "fulfilled", "printing"].includes(order.status)) return new Response("Not found", { status: 404 });
  const key = kind === "outside" ? order.outside_asset_key : order.inside_asset_key;
...
async function prodigiProductDetails(env) {
  ...
  const response = await fetch(`${base}/v4.0/products?skus=${encodeURIComponent(sku)}`, {
    headers: { "X-API-Key": env.PRODIGI_API_KEY },
  });
  const payload = await response.json();
  if (!response.ok) throw new Error(`Prodigi product lookup failed: ${response.status}`);
  return payload;
}
...
async function submitToProdigi(order, env) {
  ...
  const outsideUrl = await makePrintAssetUrl(order.id, "outside", env);
  const insideUrl = await makePrintAssetUrl(order.id, "inside", env);
  const product = await prodigiProductDetails(env);
  const areas = extractPrintAreas(product);
  if (!areas.length) throw new Error("Could not determine Prodigi greeting-card print areas. Check the configured SKU.");
  ...
  if (!insideArea) throw new Error(`The configured Prodigi SKU does not expose an inside print area (${areas.join(", ")}). Hold this order and choose the personalised-inside SKU before going live.`);
  const base = env.PRODIGI_ENV === "live" ? "https://api.prodigi.com" : "https://api.sandbox.prodigi.com";
  ...
  const payload = {
    ...
    items: [{
      ...
      assets: [
        { printArea: outsideArea, url: outsideUrl },
        { printArea: insideArea, url: insideUrl },
      ],
    }],
  ...
  if (!response.ok || !result?.order?.id && !result?.id) throw new Error(`Prodigi order failed: ${result?.outcome || response.status}`);
  return { providerOrderId: result?.order?.id || result.id, raw: result };
}
...
async function stripeWebhook(request, env, ctx) {
  ...
  if (event.type === "checkout.session.completed" && event.data?.object?.payment_status === "paid") {
    ...
    if (orderId && env.DB) {
      ...
      await env.DB.prepare(`UPDATE orders SET status='paid', stripe_payment_intent=?, customer_email=?, shipping_json=?, updated_at=? WHERE id=? AND status IN ('checkout','draft')`)
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/config") {
        ...
        return json({
          ...cfg,
          checkoutReady: Boolean(env.STRIPE_SECRET_KEY && env.DB && env.ASSET_BUCKET),
      ...
      if (url.pathname === "/api/stripe-webhook" && request.method === "POST") return stripeWebhook(request, env, ctx);
