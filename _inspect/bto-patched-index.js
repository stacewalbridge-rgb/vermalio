...
async function createPrintAssetUrls(orderId, env) {
  ...
  const make = (kind) => `${base}/api/print-asset/${encodeURIComponent(orderId)}/${encodeURIComponent(kind)}?token=${encodeURIComponent(token)}`;
  return { outsideUrl: make("outside"), insideUrl: make("inside") };
}
