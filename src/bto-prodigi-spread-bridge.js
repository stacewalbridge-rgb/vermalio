...
async function assetCapabilityAuthorized(order, outsideUrl) {
  const parsed = parseBtoAssetUrl(outsideUrl);
  if (!parsed || parsed.kind !== "outside" || parsed.orderId !== String(order?.id || "")) return false;
  ...
  if (!(contentType.includes("image/png") || contentType.includes("image/jpeg"))) return false;
  ...
  try {
    if (request.method === "GET" && url.pathname.endsWith("/health")) {
      ...
      return json({
        ...
        printArea: "default",
        horizontalResolution: 6118,
        verticalResolution: 2161,
      });
    }
    ...
    const capabilityOk = bearerOk ? true : await assetCapabilityAuthorized(order, body?.outsideUrl);
    if (!capabilityOk) return json({ ok: false, error: "forbidden" }, 403);

    const parsedOutside = parseBtoAssetUrl(body?.outsideUrl);
    if (!parsedOutside) throw new Error("Invalid Built To Offend spread artwork URL");
