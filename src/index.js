...
async function orderStatus(url, env, ctx) {
  ...
  if (["paid", "fulfilment_retry"].includes(order.status) && ctx) ctx.waitUntil(fulfillOrder(order.id, env).catch(() => {}));
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/order-status" && request.method === "GET") return orderStatus(url, env, ctx);
